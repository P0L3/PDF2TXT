...
doctype5_1 = {
    ...
    "get_authors_and_affiliations_nu": ["font-family: AdvMacms; font-size:9px"],   # Number
    "get_authors_and_affiliations_af": ["font-family: AdvOTb92eb7df.I; font-size:6px"],  # Affiliation text
...
styles = [doctype1_1, doctype2_1, doctype3_1, doctype4_1, doctype5_1, doctypedef_1]

data_list = []
Faults = 0
Faulty_samples = []
Styleless_samples = []
skip_samples = ["Editorial", "Erratum", "JEPO"]

samples = listdir(DIR)
# samples = ["Carbon-capture-and-storage_2008_Energy-Policy.pdf"]
for sample in tqdm(samples[20:]):
    ...
    # Skip pdf-s starting with a certain term
    should_skip = any(sample.startswith(skip) for skip in skip_samples)
    if should_skip:
        print(f"Skipping {sample.split()[0]} board pdf: {sample}")
        continue
    ...
    while len(title[0]) == 0:
        ...
        title[0] = title[0].replace("Energy Policy", "").replace("Palaeogeography, Palaeoclimatology, Palaeoecology", "")  # Quick fix
